...
exports.create = exports.load = function(opts, callback) {
  ...
  auth(opts, function(err, token) {
    ...
    spreadsheet.init(token, callback);
  });
};
    ...
    _this.raw[type+'sheets'] = entries.map(function(e1) {
      ...
      if(_this.opts.useDefaultWorksheet && type == "work") { // patch
        if(!_this[id] && e2.id && /([^\/]+)$/.test(e2.id)) {
          ...
          console.log('using default worksheet: ' + e2.title)
        }  
      } else {
    ...
    function send(next){
      _this.request({
        ...
        if(result.indexOf("success='0'") >= 0)
          if(result.indexOf("a response has already been sent for batch operation") >= 0)
            return next("Update failed. Your update may not fit in the worksheet. See the 'autoSize' option.");
          else
            return next("Unknown error: " + result);
